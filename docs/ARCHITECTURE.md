...
**`server/subagents.py`** — the sub-agent runs beside a Claude Code transcript. Reads `<session-id>/subagents/agent-*.jsonl` and its `.meta.json` sidecar. Two entry points, split by cost: `latest_mtime` is one `scandir` with no parsing and runs for every session on every board poll; `for_session` parses metadata and is reserved for the detail view. Which runs are *live* is not decided here — it comes from the main transcript's unanswered `Agent`/`Task` calls. `current_task` scopes a roster to the newest task (runs dispatched since the last user prompt, plus anything still running) so the panel reads as live delegation rather than a session-long log; every backend's roster goes through it, and retired runs stay readable by id.
